...
Daily-post banners are generated by `script/daily_post/make_banner.py`, which authors the
artwork as **SVG** and rasterises it with `cairosvg` — an optional but preferred dependency
(`pip install -r script/daily_post/requirements.txt`). On that path the vector source ships
alongside the rasters as `assets/images/<slug>/banner.svg`, so the banner stays editable
after publication; it is not referenced by any post, and `script/check_frontmatter.py` is
fine with that. When `cairosvg` cannot be imported — a scheduler box without `libcairo`, say
— the script prints a note to stderr, renders the same banner with Pillow instead, and exits
`0`. Only the `.webp` derivatives are ever referenced from front matter.
